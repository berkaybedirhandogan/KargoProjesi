...
namespace KargoTakibi.Controllers
{
    ...
    public class HareketController : Controller
    {
        ...
        public async Task<IActionResult> KargoHareketleri(int? id)
        {
            ...
            if (kargo != null)
            {
                ViewBag.GondericiSehir = kargo.GondericiIl?.Split('/')[0].Trim();
                ViewBag.AliciSehir = kargo.AliciIl?.Split('/')[0].Trim();
            }

            ViewBag.GondericiSehir = kargo.GondericiIl;
            ViewBag.AliciSehir = kargo.AliciIl;
